...
func main() {
	...
	hdl := handler.NewAMQPHandler(svc)

	rabbitmqURL := getEnv("RABBITMQ_URL", "amqp://guest:guest@127.0.0.1:5672/")
	bindAddr := getEnv("BIND_ADDR", "127.0.0.1:8084")
	...
	if err := hdl.StartConsumer(ctx, rabbitmqURL); err != nil {
		log.Fatalf("Notification Service failed to start RabbitMQ consumer: %v", err)
	}
	...
	log.Printf("Notification Service starting on %s...", bindAddr)
